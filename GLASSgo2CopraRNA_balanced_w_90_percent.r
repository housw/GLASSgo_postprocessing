...
sim<-min(ceiling(max_number/2),15)
...
if(nrow(coor2)>max_number){
	...
	pre<-c(coor2[pos_wild,"fin"],sel)
	out<-c()
	for(i in 1:length(clus2)){
		temp<-clus2[[i]]
		temp_pre<-na.omit(match(pre,names(temp)))
		if(length(temp_pre)>0){
			temp<-temp[-temp_pre]
		}
